...
Create `atsc/tests/v1_wire_compat.rs` with one test per codec. Each test must recompress the same input, compare it to `include_bytes!("fixtures/v1/<codec>.bro")`, parse the fixture, and compare decompressed output. Use exact equality for Constant, Noop, and RLE; use `calculate_error` with `<= 0.05` for FFT, Polynomial, and IDW.
