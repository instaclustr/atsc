...
if [[ $# -lt 1 ]]; then
  echo "usage: $0 INPUT.wbro [--strict]"
  exit 2
fi
...
STEM="${STEM%.wbro}"
NAME="$STEM"
if [[ "$STEM" == *heap* ]]; then NAME="heap"; fi
if [[ "$STEM" == *memory* ]]; then NAME="memory"; fi
if [[ "$STEM" == *iowait* ]]; then NAME="iowait"; fi
...
cd "$ROOT"
CARGO_TARGET_DIR="$ROOT/target" cargo build -q --locked --manifest-path "$ROOT/tools/Cargo.toml" --bin wbro2f64
CARGO_TARGET_DIR="$ROOT/target" cargo build -q --locked -p atsc-cli

WBRO2F64="$ROOT/target/debug/wbro2f64"
ATSC="$ROOT/target/debug/atsc-cli"

"$WBRO2F64" "$INPUT" "$TMP/input.f64"
